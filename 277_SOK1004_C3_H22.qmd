...
# Samler alle verdiene (fjerner alle NA verdiene) med samme dato uavhengig av hvilken statistikkvariabel og konsumgruppe de har og summerer disse sammen. 
df_total <- aggregate(df_copy_2["verdi"], by = df_copy_2["dato"], sum, na.rm = TRUE)
...
*Når vi leser av output'en over kan vi se at det største avviket er på 0.0514 (1 - 0.9486) og det minste avviket er på 0.0233 (1 - 0.9767).*
...
# Samler alle verdiene (fjerner alle NA verdiene) med samme dato uavhengig av hvilken statistikkvariabel og konsumgruppe de har og finner gjennomsnittet. 
df_totalindeks <- aggregate(df_copy_3["verdi"], by = df_copy_3["dato"], mean, na.rm=TRUE)
```

*Her har jeg laget en totalindeks som inneholder gjennomsnittsverdiene til alle konsumgruppene til den gitte måneden det året (fra 2011 til 2022). For å stole på om det er riktig kan jeg se på året 2015 for å se at verdiene ligger på rundt 100 siden dette er året KPI totalindeksen tar utgangspunkt i.*
...
*Vi kan se prisen på elektrisitet målt for inflasjon med standaråret 2015. Som vi ser er strømmen på sitt dyreste i 2022 og på sitt billigste rundt 2012/2013 uavhengig av inflasjon. I starten av 2022 var strømmen dobbelt så dyr som i 2015.*

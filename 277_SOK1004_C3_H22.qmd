...
# Lager en ny "df" som kun inneholder "Konsumprisindeks(vekter)" og deler verdiene på 1000. 
df_copy_2 <- df_copy %>%
  filter(variabel == "Konsumprisindeks (vekter)") %>%
  mutate (verdi = verdi / 1000)

# Legger sammen alle verdiene med samme dato uavhengig av hvilken statistikkvariabel og konsumgruppe de har. 
df_total <- aggregate(df_copy_2$verdi, by = list (df_copy_2$dato), FUN = sum, na.rm = TRUE)

# Gir variablene i "df_total" nye navn.
df_total <- df_total %>%
  rename("dato" = "Group.1") %>%
  rename("total" = "x")

# Bruker "summary" for å få minsteverdien og maximum verdien slik at jeg kan finne aviket i forhold til 1.
df_total %>%
  summary()
...
Zach, 05.04.2022, How to use as.Date() function in R (with examples), statology, hentet 18.10.2022 fra
